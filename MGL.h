...
MGLDEF void gpset(float x1, float y1, uint32_t c);
...
void draw_and_vsync() {
    ...
    for (int i = 0; i < GRP_N; i++) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, GRP_W, GRP_H, 0, GL_RGBA, GL_UNSIGNED_BYTE, vram[i]);
    }
...
int MGL_Init() {
    ...
    for (int i = 0; i < GRP_N; i++) {
        ...
        vram[i] = calloc(sizeof(uint32_t), vram_size_n);
        if (!vram[i]) {
            fprintf(stderr, "Cannot allocate vram (%dbytes) for GRP%d\n", sizeof(uint32_t) * vram_size_n, i);
            return -1;
        }
